...
Canonical pattern guidance lives in `~/.claude/custom/KNOWLEDGE/swift/patterns/`.
...
2. Read only the matching `~/.claude/custom/KNOWLEDGE/swift/patterns/<category>/` file after a signal appears.
...
```bash
# Scan by category
ls ~/.claude/custom/KNOWLEDGE/swift/patterns/

# Grep in the project
grep -rn "URLSession.shared\|[String: Any]\|Thread.sleep\|attributesOfItem" --include="*.swift" Sources/ Tests/

# Read the file on match
sed -n '1,220p' ~/.claude/custom/KNOWLEDGE/swift/patterns/performance/_summary.md
```
