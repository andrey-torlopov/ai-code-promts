...
**MANDATORY: Use Codex multi-agent collaboration for semantic chunks. Reading files yourself one-by-one is forbidden because it defeats the parallel extraction design.**
...
**Step B2 - Dispatch subagents before waiting**

Call `spawn_agent` once per chunk up to the runtime's available concurrency slots. Dispatch every
agent in the current batch before calling `wait_agent`; then dispatch the next batch when slots are
free. Do not spawn one agent, wait for it, and only then spawn the next.

Each subagent needs shared-filesystem write access because the chunk file on disk is the success
signal. Give it one bounded chunk and one absolute output path; do not delegate orchestration or
merge work.

Concrete example for 3 chunks:
```
[spawn_agent call 1: files 1-15, task_name="graphify_chunk_01"]
[spawn_agent call 2: files 16-30, task_name="graphify_chunk_02"]
[spawn_agent call 3: files 31-45, task_name="graphify_chunk_03"]
```
Dispatch all three before the first wait when the runtime has enough slots; otherwise use bounded
batches without exceeding the available concurrency.
...
- If the file is missing, print a warning: "chunk N missing from disk — subagent did not persist its result." Do not silently skip.
- If a subagent failed or returned invalid JSON, print a warning and skip that chunk - do not abort

If more than half the chunks failed or are missing, stop and report that semantic extraction did
not produce enough persisted chunk results to merge safely.

Merge all chunk files into `.graphify_semantic_new.json`. If the collaboration result exposes real
input/output token usage, write it into the chunk JSON before merging. If the runtime does not
expose usage, keep both counts at zero and report token metrics as unavailable rather than
estimating them. Then run:
```bash
$(cat graphify-out/.graphify_python) -c "
import json, glob
from pathlib import Path
