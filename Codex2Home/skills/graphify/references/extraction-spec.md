...
Then write the JSON to disk using the available filesystem editing tool at this exact absolute path (no relative paths — subagents may not share the parent's working directory):
CHUNK_PATH
```
