...
1. Read `../../RESOLVER.md` only if the mode is not already clear.
2. Read the selected `modes/<mode>.md`.
3. Read only the references named by that mode.
4. Load only the minimum `../../KNOWLEDGE/` packs selected by the mode and scope.
...
For substantial work, finish with `TRACE` from `../../_core/skill-context.md`.
