...
Load `KNOWLEDGE/swift` and `KNOWLEDGE/ios` for Xcode, Swift, iOS, macOS, `.xcodeproj`, `.xcworkspace`, `Package.swift` or `Package.resolved` work. Skip unrelated knowledge packs unless the project scope requires them.
...
Every substantial run ends with the final `TRACE` block from `../../RESOLVER.md`, reporting references read, knowledge read, patterns or policies applied, verification and residual risk.
